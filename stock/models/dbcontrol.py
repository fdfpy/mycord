...
CSV_COLUMNS = ['STOCK_NUM', 'COM_NAME', 'URL', 'SIGMA', 'EXPECTATION', 'KESSAN', 'W2W3', 'W1W2', 'TODAY', 'PER', 'LASTYEAR_PROFIT_PER_STOCK', 'THISYEAR_PROFIT_PER_STOCK', 'KESSAN_MONTH', 'PERMAX', 'PERMIN','KAIRI_MIN', 'KAIRI_MAX','KAIRI_RANK','KINOU']
...
class DBCONT(object):
    ...
    def __init__(self, data):
        ...
        #s_ototoi_yesterday =data[4]   #2week前-3week前 週足株価の計算を含む 
        s_2w3w_dif =data[4]   #2week前-3week前 週足株価の計算を含む        
        #s_yesterday_today =data[3]    #1week前-2week前 週足株価の計算を含む 
        s_1w2w_dif =data[3]    #1week前-2week前 週足株価の計算を含む 

        #print("2week前-3week前")
        #print(s_ototoi_yesterday)   #2week前-3week前 週足株価の計算を含む 
        #print("1week前-2week前")     
        #print(s_yesterday_today)    #1week前-2week前 週足株価の計算を含む 
        ...
        comb = (v_sigma, v_exp, s_num, s_2w3w_dif, s_1w2w_dif, per_now, today, per_max, per_min,kairi_min,kairi_max,kinou)
        #print("self.comb")
        #print(comb)
        ##self.dbkakikomi(v_sigma, v_exp, str(s_num), int(s_ototoi_yesterday), int(s_yesterday_today), per_now, today, per_max, per_min)
        self.dbkakikomi(v_sigma, v_exp, str(s_num), int(s_2w3w_dif), int(s_1w2w_dif), per_now, today, per_max, per_min,kairi_min,kairi_max,kairi_rank,kinou)
    ...
    def dbkakikomi(self,v_sigma,v_exp,s_num,s_2w3w_dif,s_1w2w_dif,per_now,today,per_max,per_min,kairi_min,kairi_max,kairi_rank,kinou):  #各銘柄のボラティリティーをDBに追加する関数
        ...
        insert_sql='UPDATE STOCK_INFO SET SIGMA=?, EXPECTATION=?,W2W3=?,W1W2=?,TODAY=?,PER=?,PERMAX=?,PERMIN=?,KAIRI_MIN=?,KAIRI_MAX=?,KAIRI_RANK=?,KINOU=? WHERE STOCK_NUM=?'
        ##cur.execute(insert_sql,(v_sigma,v_exp,s_ototoi_yesterday,s_yesterday_today,today,per_now,per_max,per_min,s_num))
        cur.execute(insert_sql,(v_sigma,v_exp,s_2w3w_dif,s_1w2w_dif,today,per_now,per_max,per_min,kairi_min,kairi_max,kairi_rank,kinou,s_num))
        print("#")
        con.commit()
        self.db_dump(cur)
        con.close()
